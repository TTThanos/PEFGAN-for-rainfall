...
model_DESRGAN = 'vOriginal_DESRGAN'
DESRGAN_num = 'model_G_i000003'
model_PEFGAN = 'vdynamic_weights'
PEFGAN_num = 'model_G_i000008'

qm_mask = np.load("/scratch/iu60/yl3101/QM(AWAP)_mask/awap_binary_mask.npy")
def plot_fortnight_diff(year, lead, window):
    avg_fortnight_DESRGAN = []
    avg_fortnight_PEFGAN = []
    for time in (range(0, lead)):
        ...
        leading0_model_DESRGAN = np.load(
            "/scratch/iu60/yl3101/PEFGAN/new_crps/save/crps_ss/" + model_DESRGAN + "/" + year + '/' + DESRGAN_num + "/lead_time" + str(time) + "_whole.npy")
        leading0_model_DESRGAN = np.ma.masked_array(leading0_model_DESRGAN, mask=qm_mask)

        crpsss_v3_DESRGAN = 1 - (leading0_model_DESRGAN / leading0_clima)
        avg_fortnight_DESRGAN.append(crpsss_v3_DESRGAN)

        leading0_model_PEFGAN = np.load(
            "/scratch/iu60/yl3101/PEFGAN/new_crps/save/crps_ss/" + model_PEFGAN + "/" + year + '/' + PEFGAN_num + "/lead_time" + str(time) + "_whole.npy")
        leading0_model_PEFGAN = np.ma.masked_array(leading0_model_PEFGAN, mask=qm_mask)

        crpsss_v3_PEFGAN = 1 - (leading0_model_PEFGAN / leading0_clima)
        avg_fortnight_PEFGAN.append(crpsss_v3_PEFGAN)
        ...
        if time == 13:
            first_14 = np.mean(avg_fortnight_PEFGAN, axis = 0) - np.mean(avg_fortnight_DESRGAN, axis = 0)
            draw_aus(first_14.squeeze(), mode='crps-ss', lat=lat, lon=lon, title="CRPS_SS_diff_1st_DESRGAN/PEFGAN",
                             titles_on=True,
                             save=True,
                             path="/scratch/iu60/yl3101/PEFGAN/new_crps/image/window_1/crps_ss/" + year + "/1st fortnight_diff.jpeg")
        if time == 27:
            second_14 = np.mean(avg_fortnight_PEFGAN[14:], axis = 0) - np.mean(avg_fortnight_DESRGAN[14:], axis = 0)
            draw_aus(second_14.squeeze(), mode='crps-ss', lat=lat, lon=lon, title="CRPS_SS_diff_2nd_DESRGAN/PEFGAN",
                             titles_on=True,
                             save=True,
                             path="/scratch/iu60/yl3101/PEFGAN/new_crps/image/window_1/crps_ss/" + year + "/2nd fortnight_diff.jpeg")
        if time == 41:
            third_14 = np.mean(avg_fortnight_PEFGAN[28:], axis = 0) - np.mean(avg_fortnight_DESRGAN[28:], axis = 0)
            draw_aus(third_14.squeeze(), mode='crps-ss', lat=lat, lon=lon, title="CRPS_SS_diff_3rd_DESRGAN/PEFGAN",
                             titles_on=True,
                             save=True,
                             path="/scratch/iu60/yl3101/PEFGAN/new_crps/image/window_1/crps_ss/" + year + "/3rd fortnight_diff.jpeg")

plot_fortnight_diff("2006", lead = 42, window = 1)
plot_fortnight_diff("2007", lead = 42, window = 1)
plot_fortnight_diff("2015", lead = 42, window = 1)            
plot_fortnight_diff("2018", lead = 42, window = 1)