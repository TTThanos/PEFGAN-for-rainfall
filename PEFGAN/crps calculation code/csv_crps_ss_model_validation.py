...
def evaluate(year, lead, draw=False, window=1):
    ...
    evaluate CRPS-SS
    '''
    model_names = ['model_G_i000001','model_G_i000002','model_G_i000003','model_G_i000004','model_G_i000005','model_G_i000006','model_G_i000007',
                   'model_G_i000008','model_G_i000009','model_G_i000010']
    for model_num in model_names:
        ...
        with open(model_file_csv, "w", newline='') as file:
            csv_file = csv.writer(file)
            head = ["lead time", "PUGAN_crps", "PUGAN_crpsss", "QM_crpsss", "mae", "bias", "relative_bias", "Brierscore_0", "Brierscore95", "Brierscore99"]
            csv_file.writerow(head)
            ...
            for time in (range(0, lead)):
                ...
                if draw:
                    ...
                    if not os.path.exists("/scratch/iu60/yl3101/PEFGAN/new_crps/image/" + model_name + "/" + str(year) + "/window_" + str(
                            ...
                            os.mkdir("/scratch/iu60/yl3101/PEFGAN/new_crps/image/" + model_name + "/" + str(year) + "/window_" + str(
                                    window) + '/' + model_num)       
                    draw_aus(crpsss_v3[0], mode='crps-ss', lat=lat, lon=lon, title="CRPS_SS_DESRGAN_" + str(time + 1),
                                titles_on=True,
                                save=True,
                                path="/scratch/iu60/yl3101/PEFGAN/new_crps/image/" + model_name + "/" + str(year) + "/window_" + str(
                                    window) + '/' + model_num + "/DESRGAN_" + str(time + 1) + ".jpeg")
                    # if time== 7 or time == 14 or time == 29 or time == 44 or time == 89 or time == 179 or time == 216:
                        ...
                        # draw_aus(total_save, mode='crps-ss', lat=lat, lon=lon, title="CRPS_SS_DESRGAN_" + str(time + 1),
                        #          titles_on=True,
                        #          save=True,
                        #          path="/scratch/iu60/yl3101/DESRGAN/new_crps/image/window_" + str(
                        #              window) + "/crps_ss/" + year + "/DESRGAN_" + str(
                        #              time + 1) + ".jpeg")
