...
2.using my net to train one channel to one channel.
    ...
    def __init__(self, start_date=date(1990, 1, 1), end_date=date(1990, 12, 31), regin="AUS", lr_transform=None,
        ...
        cali_date, date_for_AWAP, time_leading = self.filename_list[0]
        if shuffle:
    ...
    def get_filename_with_time_order(self, rootdir):
        ...
        for date in self.dates:
            for i in range(self.leading_time_we_use, self.leading_time_we_use + 1):
                access_path = rootdir +"/" +  date.strftime("%Y%m%d") + ".nc"
                    #                   print(access_path)
    ...
    def __getitem__(self, idx):
        ...
        access_date, awap_date, time_leading = self.filename_list[idx]
        en="e01"
...
def main(year, days):
    ...
    parser.add_argument('--file_ACCESS_dir', type=str, default="/scratch/iu60/rh4668/VDSR_crps/eval/2002/", help='dataset directory')
    ...
    for lead in range(0, days):
        ...
        for batch, (pr, hr,  _, _, _) in enumerate(test_data):
            ...
            with torch.set_grad_enabled(False):
                ...
                for i in range(args.batch_size):
                    a = np.squeeze(sr_np[i])
                    b = np.squeeze(hr_np[i])
                    print(a.shape)
                    mae_BI = mae(a, b)
                    bias_BI = bias(a, b)
        ...
        if not os.path.exists("/scratch/iu60/rh4668/score/save/mae/VDSR_crps/" + str(year)):
            os.mkdir("/scratch/iu60/rh4668/score/save/mae/VDSR_crps/" + str(year))
        np.save("/scratch/iu60/rh4668/score/save/mae/VDSR_crps/" + str(year) + "/lead_time" + str(lead) + '_whole',
                np.mean(mean_mae_model, axis=0))

        if not os.path.exists("/scratch/iu60/rh4668/score/save/bias/VDSR_crps/" + str(year)):
            os.mkdir("/scratch/iu60/rh4668/score/save/bias/VDSR_crps/" + str(year))
        np.save("/scratch/iu60/rh4668/score/save/bias/VDSR_crps/" + str(year) + "/lead_time" + str(lead) + '_whole',
                np.mean(mean_bias_model, axis=0))

        if not os.path.exists("/scratch/iu60/rh4668/score/save/bias_median/VDSR_crps/" + str(year)):
            os.mkdir("/scratch/iu60/rh4668/score/save/bias_median/VDSR_crps/" + str(year))
        np.save("/scratch/iu60/rh4668/score/save/bias_median/VDSR_crps/" + str(year) + "/lead_time" + str(lead) + '_whole',
                np.mean(mean_bias_median_model, axis=0))

        if not os.path.exists("/scratch/iu60/rh4668/score/save/rmse/VDSR_crps/" + str(year)):
            os.mkdir("/scratch/iu60/rh4668/score/save/rmse/VDSR_crps/" + str(year))
        np.save("/scratch/iu60/rh4668/score/save/rmse/VDSR_crps/" + str(year) + "/lead_time" + str(lead) + '_whole',
                np.mean(mean_rmse_model, axis=0))

        if not os.path.exists("/scratch/iu60/rh4668/score/save/crps_ss/VDSR_crps/" + str(year)):
            os.mkdir("/scratch/iu60/rh4668/score/save/crps_ss/VDSR_crps/" + str(year))
        np.save("/scratch/iu60/rh4668/score/save/crps_ss/VDSR_crps/" + str(year) + "/lead_time" + str(lead) + '_whole',
                np.mean(mean_crps_model, axis=0))

        if not os.path.exists("/scratch/iu60/rh4668/score/save/mae_mean/VDSR_crps/" + str(year)):
            os.mkdir("/scratch/iu60/rh4668/score/save/mae_mean/VDSR_crps/" + str(year))
        np.save("/scratch/iu60/rh4668/score/save/mae_mean/VDSR_crps/" + str(year) + "/lead_time" + str(lead) + '_whole',
                np.mean(mae_mean_score_model, axis=0))

        if not os.path.exists("/scratch/iu60/rh4668/score/save/mae_median/VDSR_crps/" + str(year)):
            os.mkdir("/scratch/iu60/rh4668/score/save/mae_median/VDSR_crps/" + str(year))
        np.save("/scratch/iu60/rh4668/score/save/mae_median/VDSR_crps/" + str(year) + "/lead_time" + str(lead) + '_whole',
